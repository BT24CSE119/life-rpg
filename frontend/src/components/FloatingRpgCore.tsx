  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ boxShadow: '0 0 25px rgba(6,182,212,0.12), inset 0 0 15px rgba(6,182,212,0.06)' }}
        aria-hidden="true"
      >
      ...
      {/* ── 3D Character Circular Viewport ───────────────────────────────────── */}
      <div className="relative z-10 w-52 h-52 sm:w-60 sm:h-60 rounded-full overflow-hidden border-2 border-cyan-500/30 bg-rpg-surface/85 backdrop-blur-md shadow-2xl transition-all duration-500 group-hover:border-amber-400/60 group-hover:shadow-[0_0_30px_rgba(245,200,66,0.25)] flex items-center justify-center">
        ...
        {!iframeLoaded && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-rpg-surface/90 z-10 rounded-full">
            {/* Pulsing core orb */}
            <div className="relative w-16 h-16">
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-cyan-400/40 to-purple-500/40 animate-pulse" />
              <div className="absolute inset-2 rounded-full bg-gradient-to-br from-cyan-300/60 to-amber-400/60 animate-ping" style={{ animationDuration: '1.5s' }} />
              <div className="absolute inset-4 rounded-full bg-gradient-to-br from-amber-300 to-cyan-300 shadow-[0_0_20px_rgba(6,182,212,0.6)]" />
            </div>
            <p className="text-[10px] font-mono text-cyan-400/70 animate-pulse tracking-widest uppercase">
              Summoning…
            </p>
          </div>
        )}

        {/* ── Sketchfab iframe — fades in once loaded ───────────────────────── */}
        <div className="w-[125%] h-[135%] -mt-3 -mb-12 flex items-center justify-center pointer-events-auto">
          <iframe
            title="The Character"
            src={embedUrl}
            className={`w-full h-full border-0 rounded-full scale-75 translate-y-2 transition-opacity duration-700 ${
